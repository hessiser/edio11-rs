...
use windows::{
    ...
    Win32::Graphics::{
        ...
        Direct3D11::{ID3D11Device, ID3D11PixelShader, ID3D11VertexShader, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_ELEMENT_DESC, D3D11_INPUT_PER_VERTEX_DATA}, Dxgi::Common::{DXGI_FORMAT_R32G32B32A32_FLOAT, DXGI_FORMAT_R32G32_FLOAT},
    },
};

use crate::errors::OverlayError;

pub const INPUT_ELEMENTS_DESC: [D3D11_INPUT_ELEMENT_DESC; 3] = [
...
trait Shader: Sized {
    ...
    unsafe fn create_shader(device: &ID3D11Device, blob: &ShaderData) -> Result<Self, OverlayError>;
}
    ...
    unsafe fn create_shader(device: &ID3D11Device, blob: &ShaderData) -> Result<Self, OverlayError> {
        ...
        match blob {
            ShaderData::EmbeddedData(arr) => {
                ...
                output.ok_or(OverlayError::General(
...
impl CompiledShaders {
    pub fn new(device: &ID3D11Device) -> Result<Self, OverlayError> {
        if cfg!(feature = "force-compile") {
            ...
            unsafe {
                let cache = ShaderData::EmbeddedData(include_bytes!("../shaders/vertex.bin"));
                let vertex = ID3D11VertexShader::create_shader(device, &cache)?;
                let pixel = ID3D11PixelShader::create_shader(
                    device,
                    &ShaderData::EmbeddedData(include_bytes!("../shaders/pixel.bin")),
                )?;
    ...
    fn compile_shader<S: Shader>(device: &ID3D11Device) -> Result<(ID3DBlob, S), OverlayError> {
        const SHADER_TEXT: &str = include_str!("../shaders/shader.hlsl");
